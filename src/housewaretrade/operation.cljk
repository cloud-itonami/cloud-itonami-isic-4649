(ns housewaretrade.operation
  "OperationActor -- one household-goods-wholesale operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (HousewareTradeAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Consumer Product
  Safety Governor (:govern) and the rollout phase gate (:decide) before
  ...
  One graph run = one household-goods-wholesale operation (intake ->
  advise -> govern -> decide -> commit | hold | approval). No unbounded
  inner loop -- each operation is auditable and checkpointed. A
  household-order's life is advanced by MANY ops (intake / verify /
  ...
  decision to a human operator (trading supervisor / distribution-
  center operator). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:delivery/dispatch`/`:invoice/settle`
  ALWAYS reach this node when the governor is clean -- see
  `housewaretrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [housewaretrade.housewaretradeadvisor :as housewaretradeadvisor]
...
(defn build
  ...
  `housewaretrade.store/Store`).
  opts:
    :advisor      -- a `housewaretrade.housewaretradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (housewaretradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HousewareTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (housewaretradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(housewaretradeadvisor/trace request p)]})))

      ;; Consumer Product Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
