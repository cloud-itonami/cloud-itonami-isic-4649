(ns housewaretrade.phase
  "Phase 0->3 staged rollout for the household-goods-wholesale actor.

    Phase 0  read-only        -- no writes, still governor-gated.
    Phase 1  assisted-intake  -- household-order intake allowed, every
                                 write needs human approval.
    Phase 2  assisted-verify  -- adds safety-verification writes, still
                                 approval.
  ...
  real physical household goods from the wholesale distribution center
  to a counterparty and settling a real household-goods invoice (real
  money moving between counterparty and wholesaler) are the two
  real-world physical/financial acts this actor performs; both are
  always a human trading supervisor's call. `housewaretrade.governor`'s
  ...
  distinct from the household-order itself.")

(def read-ops  #{})
(def write-ops #{:order/intake :safety/verify :delivery/dispatch :invoice/settle})
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:order/intake :safety/verify}                     :auto #{}}
...
(defn verdict->disposition
  "Map a Consumer Product Safety Governor verdict to a base disposition
